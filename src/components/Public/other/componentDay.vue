<template lang="">
      ...
      <div class="sample">
        ...
        <div class="custom-select" id="myCustomSelect">
          ...
          <span class="custom-select-icons">
            <i class="bi bi-chevron-down" style="color: black;
    font-size: 17px;
    font-weight: 900;"></i>
                        </span>
          <ul class="custom-select-options hidden-all" id="custom-select-list">
            <li v-for="partenaire in yearOptions" :key="partenaire.id" @click="selectYear(partenaire.label)"> {{partenaire.label}}</li>
    ...
    function doKeyAction(whichKey) {
      ...
      // Émettre un événement pour informer le parent de la sélection
      this.$emit('selectedYear', selectedYear);
    },
      },
